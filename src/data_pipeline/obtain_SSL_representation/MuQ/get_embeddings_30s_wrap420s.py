        ...
        while True:
            ...
            try:
                ...
                for i in range(0, 10000, WRAP_SIZE):
                    if i * args.sampling_rate >= audio.shape[-1]:
                        break
                    layer_embd = defaultdict(list)
                    # j also represents second
                    for j in range(0, WRAP_SIZE, HOP_SIZE):
                        start_idx = (i + j) * args.sampling_rate
                        end_idx = min(
                            (i + j + WIN_SIZE) * args.sampling_rate, audio.shape[-1]
                        )
                    ...
                    del output, audio_seg, emdb_list
